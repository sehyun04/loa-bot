...
(function (root, factory) {
  ...
  const REF = {
    ...
    meta: {
      ...
      // "젬 포인트 N ?" 줄. 가운데 정렬이라 자릿수에 따라 4px 쯤 밀리므로
      // 고정 창이 아니라 "젬 포인트" 이름을 템플릿으로 찾고 그 오른쪽을 읽는다.
      gemPoint:  { dx: 30, dy: -354, w: 160, h: 26 },
      /*
       * "가공 비용" 줄의 금액. 오른쪽 정렬이라 자릿수가 달라도 끝이 dx 353 에 고정이고,
       * 동전 아이콘은 dx 359 부터라 창 밖이다.
       *
       * 금액은 자릿수별로 쪼개지 않고 통째로 분류한다 - 후보가 닫힌 집합(0 / 900 /
       * 1,800)이라 조회 문제이고, 오른쪽 정렬 덕분에 창 안에서 위치도 고정이다.
       * 쪼개면 오히려 "900" 의 마지막 0 이 "0" 으로 읽히는 함정이 생긴다.
       */
      costAmount: { dx: 290, dy: 118, w: 66, h: 26 },
    },
  };
