...
| 자리 | 누가 말하나 |
|---|---|
| 화면에 뜨는 문장 전부 - 제목, 본문, `-#` 각주, 오류, 되묻기 | 니나브 (**반말**) |
| 슬래시 커맨드 설명(`description`/`describe`), 로그, 예외 이름 | 시스템 |

**반말이 기본이다.** 니나브는 원작에서 모험가에게 반말을 쓴다. 처음엔 카드가 전부
해요체라는 이유로 존댓말로 뒀는데, 그러면 캐릭터의 생기가 통째로 사라지고 책 읽는
소리가 난다. 존댓말은 원작에서도 신을 부를 때만 쓴다.

슬래시 커맨드 설명은 디스코드가 커맨드 목록 UI에 그리는 메타데이터지 봇의 말이
아니다. 거기까지 반말로 바꾸면 목록이 어수선해진다.

니나브가 말하는 자리에서 피할 것:

- **명사구 제목** - "조회 실패" 는 라벨이지 대사가 아니다. "가져오지 못했어".
- **명령형 지시** - "다시 시도해주세요", "정확히 입력해주세요" 는 시스템 말투다.
  부탁으로 바꾼다. "한 번만 더 봐줄래?".
- **수동태** - "등록되어 있지 않아요" -> "걸어둔 적이 없어".
