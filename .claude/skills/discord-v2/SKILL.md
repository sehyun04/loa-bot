...
### 줄 사이를 아주 조금만 벌리고 싶을 때는 구분선이 아니다

한 덩어리 텍스트를 개행으로 이으면 줄이 붙어서 안 읽히고, 빈 줄을 끼우면 한 줄이 통째로
비어서 과하다. `Separator` 는 그보다도 크다. **가장 작은 간격은 한 줄을 각각
`TextDisplay` 로 쪼개는 것**이다 — 컴포넌트 사이 기본 여백이 V2 에서 줄 수 있는 최소
단위다.

대신 줄 하나가 컴포넌트 하나를 먹어서 4000자보다 **40개 상한이 먼저 찬다.** 목록을
이렇게 그릴 거면 예산을 글자 수가 아니라 컴포넌트 수로 잡아라
(`character_view.MAX_COMPONENTS` 참고 — 서버 제목·구분선·"외 N캐릭" 몫까지 세고 채운다).
